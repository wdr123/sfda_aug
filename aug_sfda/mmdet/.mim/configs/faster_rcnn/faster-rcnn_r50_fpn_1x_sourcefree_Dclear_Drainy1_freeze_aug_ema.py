_base_ = [
    '../_base_/models/faster-rcnn_r50_fpn_aug.py',
     '../_base_/datasets/dusk_rainy1.py',
...
style_stats_path_weak = './data/aug_to_dayclear/night_rainy'
# style_stats_path = './data/aug_to_dayclear/dusk_rainy'
# style_stats_path = './data/aug_to_dayclear/night_sunny'
style_stats_path_strong = './data/aug_to_dayclear/night_rainy' #'./data/night_rainy_aug/night_sunny'
model = dict(
    ...
    roi_head=dict(
        bbox_head=dict(
            ...
            style_stats = style_stats_path_strong)))
src_model = dict(
    backbone=dict(frozen_stages=4,init_cfg=None),
    roi_head=dict(
        bbox_head=dict(
            num_classes=7,
            loss_bbox=dict(type='SmoothL1Loss', beta=1.0, loss_weight=1.0),
            style_stats = style_stats_path_weak)))
# optimizer
...
# actual epoch = 8 * 8 = 64
train_cfg = dict(type='SingleSourceEMAEpochBasedTrainLoop', ckpt_list=src_ckp_list,num_src=1, max_epochs=2)
