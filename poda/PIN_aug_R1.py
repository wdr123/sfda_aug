...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='cityscapes',
                        choices=['cityscapes','gta5', 'dusk_rainy', 'night_sunny', 'foggy_city'], help='Name of dataset')
    ...
    parser.add_argument("--domain_desc", type=str , default = "driving at clear night times",
                        help = "description of the target domain")
    parser.add_argument("--alpha", type=float , default = 0.5,
                        help = "regularizer R1 hyperparameter")
...
def main():
    ...
    model = network.modeling.__dict__[opts.model](num_classes=19,BB= opts.BB,replace_stride_with_dilation=[False,False,False])#num_classes=19
    ...
    for i,(img_id, tar_id, images, labels) in enumerate(train_loader):
            ...
            # model_pin_1 = PIN([f1.shape[0],512,1,1],f1.to(device)) #  mu_T (B,C1)  sigma_T(B,C1)
            ...
            while cur_itrs< opts.total_it: 
                ...
                #loss
                # breakpoint()
                original_feature_f1 = model.backbone(t1(f1),trunc1=True,trunc2=False,trunc3=False,trunc4=False,get1=False,get2=False,get3=False,get4=False)
                original_feature_f1 /= original_feature_f1.norm(dim=-1, keepdim=True).clone().detach()
                # loss_CLIP1 = (1- torch.cosine_similarity(text_target, target_features_from_f1, dim=1)).mean() + 0.99 * torch.norm(torch.absolute(target_features_from_f1 - original_feature_f1), 1)
                loss_CLIP1 = (1- torch.cosine_similarity(text_target, target_features_from_f1, dim=1)).mean() + opts.alpha * torch.norm(torch.absolute(target_features_from_f1 - original_feature_f1), 1)
    ...
    # print('style vector dim:',learnt_mu_f1[k].size() )
