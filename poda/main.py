...
from datasets import Cityscapes, gta5, DWD
...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='cityscapes',
                        choices=['cityscapes','gta5', 'dusk_rainy', 'night_sunny'], help='Name of dataset')
...
def get_dataset(dataset,data_root,crop_size,ACDC_sub="night",data_aug=True):
    ...
    if dataset == 'dusk_rainy' or dataset == 'night_sunny' or dataset == 'foggy_city':
        if data_aug:
            train_transform = et.ExtCompose([
                et.ExtRandomCrop(size=(crop_size, crop_size)),
                et.ExtColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
        ...
        train_dst = DWD(root=data_root,dataset=dataset,
                               split='train', transform=train_transform)
        val_dst = DWD(root=data_root,dataset=dataset,
                             split='val', transform=val_transform)
        ...
        for (im_id, tg_id, images, labels) in train_loader:
            ...
            if opts.train_aug:
                ...
                outputs,features = model(images,mu_t_f1.to(device),std_t_f1.to(device),
                                    transfer=True,mix=opts.mix,activation=relu)#network.utils._segmantation
...
if __name__ == '__main__':
    ...
    # # Set up metrics
    # metrics = StreamSegMetrics(19)
    # score = validate(opts, model, val_loader, device, metrics)
    # print(score)